...
def _expected_stow_bytes(files: List[Any]) -> int:
    total = 0
    for entry in files:
...
def _write_stow_bytes_to_dir(
    output_dir: Path, file_name: str, data: bytes, dest_path: Path
) -> int:
    """Write one STOW file entry to ``output_dir`` (expands zip archives in place)."""
...
def _parallel_download_send_files_to_dir(
    ...
    *,
    ...
    for index, file_name, data in assembled:
        ...
        byte_length = _write_stow_bytes_to_dir(
...
def _plan_send_file_jobs_to_dir(
    ...
    for index, entry in enumerate(files):
        if not isinstance(entry, dict):
            continue
        file_name = file_name_for_index(entry, index)
        dest_path = _stow_dest_path(output_dir, file_name, index, hub_event)
        expected = _entry_expected_byte_length(entry)
        raw = _dicom_bytes_from_resource(entry)
        if raw:
            if expected is not None and len(raw) != expected:
...
def _extract_all_send_files_to_dir(
    ...
    for _index, file_name, dest_path, raw in inline:
        total_bytes += _write_stow_bytes_to_dir(output_dir, file_name, raw, dest_path)
        resolved += 1
    ...
    expected_bytes = _expected_stow_bytes(files)
